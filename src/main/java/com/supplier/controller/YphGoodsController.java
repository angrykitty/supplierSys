...
public class YphGoodsController extends Controller {
     ...
     * 获取商品列表
     */
    public void modifyYphGoodsAndImg() {
        try {
            Integer pageNum = getParaToInt("pageNum");
            Integer pageSize = getParaToInt("pageSize");
            //标题
            ...
            //查询开始时间
            String startTime = getPara("stateTime");
            //查询结束时间
            String endTime = getPara("endTime");
            Page<YphGoods> goodsPage = yphGoodsService.queryPageGoods(title, verifyState, startTime, endTime, pageNum, pageSize);
            for (YphGoods yphGoods : goodsPage.getList()) {
                List<YphGoodsImg> list = yphGoodsImgService.queryGoodsImg(yphGoods.getId());
                yphGoods.getGoodsImgLis().addAll(list);
            }
    ...
    public void modifyYphGoodsState() {
        try {
            ...
            //审批信息
            Integer verifyState = getParaToInt("verifyState");
            if (StringUtils.isEmpty(des) || id == null || verifyState != null) {
     ...
     * 修改备注
     */
    public void modifyYphGoodsDes() {
        try {
            Integer id = getParaToInt("id");
            //备注
            String des = getPara("des");
            if (StringUtils.isEmpty(des) || id == null) {
